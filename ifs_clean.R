...
## Subset data frames based on frequency in order to properly clean headers
...
for (i in 1:length(freqs)) {
  ...
  for (j in 1:length(ifs_fred_nat)) {
    ...
    # Subset by country
    assign(country_name,frame[frame$Country.Name == values(ifs_fred_nat,USE.NAMES=F)[j],])
